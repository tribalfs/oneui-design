...
open class RoundedTabLayout @JvmOverloads constructor(
    ...
    private var previousSelectedTabPosition: Int = -1
    private var customTabViewPool: Pools.Pool<View> = Pools.SimplePool<View>(12)

    init {
        background = ContextCompat.getDrawable(context, R.drawable.oui_des_rounded_tab_layout_background)
    ...
    override fun selectTab(tab: Tab?, updateIndicator: Boolean) {
        val previouslySelectedTab = if (previousSelectedTabPosition != -1) getTabAt(previousSelectedTabPosition) else null
        super.selectTab(tab, updateIndicator)

        if (previouslySelectedTab != null && previouslySelectedTab != tab) {
            (previouslySelectedTab.tag as? TextView)?.let { updateTextStyle(it, false) }
        }
        if (tab != null) {
            (tab.tag as? TextView)?.let { updateTextStyle(it, true) }
        }

        previousSelectedTabPosition = selectedTabPosition
    }
    ...
    private fun updateTextStyle(textView: TextView, isSelected: Boolean){
        val styleResId = if (isSelected) appcompatR.style.RobotoMedium else appcompatR.style.RobotoRegular
    ...
    private fun getOrCreateCustomTabView(tab: Tab): View {
        ...
        if (TextUtils.isEmpty(tab.contentDescription)) {
