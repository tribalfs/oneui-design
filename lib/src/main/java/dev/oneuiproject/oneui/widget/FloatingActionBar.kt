...
class FloatingActionBar @JvmOverloads constructor(
    ...
    private fun initViews() {
        ...
        iconSwitcher.setFactory {
            ImageView(context).apply {
                imageTintList =
                    ColorStateList.valueOf(context.getThemeAttributeValue(androidx.appcompat.R.attr.colorPrimary)?.data!!)
            }
        }
    }
    ...
    private fun initAttrs(context: Context, attrs: AttributeSet?) {
        ...
        context.withStyledAttributes(attrs, R.styleable.FloatingActionBar) {
            ...
            button1TextView.text = getString(R.styleable.FloatingActionBar_button1Label) ?: "Button 1"
            button2TextView.text = getString(R.styleable.FloatingActionBar_button2Label) ?: "Button 2"
    ...
    private fun animateToSelected() {
        if (!refreshTranslations) {
            ...
            // Post the animation to ensure layout has occurred.
            post { animateToSelectedInternal() }
            return
        }
        animateToSelectedInternal()
    }

    private fun animateToSelectedInternal() {
        ...
        frontLayout.animate().apply {
            translationX(targetTranslation)
            duration = ANIMATION_DURATION
            interpolator = CachedInterpolatorFactory.getOrCreate(Type.PATH_0_22_0_25_0_0_1_0)
            withStartAction { updateSwitchersForSelectedIndex() }
            withEndAction { frontLayout.setLayerType(View.LAYER_TYPE_NONE, null) }
            start()
        }
    }
