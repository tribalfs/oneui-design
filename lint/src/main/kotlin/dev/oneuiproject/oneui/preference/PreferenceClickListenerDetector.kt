...
class PreferenceClickListenerDetector : Detector(), SourceCodeScanner {

    companion object {
        private const val ANDROIDX_PREFERENCE_CLASS = "androidx.preference.Preference"
        private const val ON_PREFERENCE_CLICK_LISTENER = "onPreferenceClickListener"
        private const val SET_ON_PREFERENCE_CLICK_LISTENER = "setOnPreferenceClickListener"

        val ISSUE_ON_CLICK_KTX: Issue = Issue.create(
            id = "UseOnClickKtx",
            briefDescription = "Replace `onPreferenceClickListener` with `onClick` KTX for conciseness.",
            explanation = """
                Instead of directly setting `onPreferenceClickListener`, you can use the `onClick` \
            ...
            implementation = Implementation(
                PreferenceClickListenerDetector::class.java,
        ...
        return object : UElementHandler() {
            ...
            override fun visitCallExpression(node: UCallExpression) {
                val (preferenceType, elementToReport) = node.resolveReceiverTypeAndElementById(
                    SET_ON_PREFERENCE_CLICK_LISTENER,
                    context,
            ...
            override fun visitBinaryExpression(node: UBinaryExpression) {
                ...
                if (node.resolveOperator()?.name == SET_ON_PREFERENCE_CLICK_LISTENER
                    && node.leftOperand is UQualifiedReferenceExpression
                ) return

                val preferenceType =
                    node.resolveReceiverTypeByIdentifier(ON_PREFERENCE_CLICK_LISTENER, context)
                        ?: return
            ...
            private fun reportIssue(
                ...
                val lintFix: LintFix = LintFix.create()
                    .name("Replace with onClick KTX")
                    .replace()
                    .range(range)
                    .with(replaceWithText)
                    .autoFix()
                    .imports("dev.oneuiproject.oneui.ktx.onClick")
                    .build()
                ...
                context.report(
                    ...
                    reportNode,
                    locationToReplace,
                    "This can be replaced with the `onClick` preference extension function.",
                    lintFix
                )
            }
        }
    ...
    private fun getReplaceWithText(
        ...
        val bodyContent = when (bodyExpression) {
            null -> "/* Place your click handling logic here */"

            is UBlockExpression -> {
                bodyExpression.joinExpressionsUnIndented(
                    omitLastStatement = bodyExpression.isLastExpressionTrueLiteral(),
                    isLambdaTarget = true
                )
            }
